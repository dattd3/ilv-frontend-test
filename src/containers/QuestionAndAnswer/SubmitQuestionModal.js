      ...
      <div ref={innerRef} {...innerProps} className="">
        <div className="d-block clearfix border-bottom py-1 btn btn-light">
          ...
          <div className="float-left text-left text-wrap w-75">
            <div className="">{props.data.fullName} ({props.data.code})</div>
            <div className="text-xs"><i>({props.data.userid}) {props.data.current_position}</i></div>
          </div>
        </div>
      </div>
    )
...
class SubmitQuestionModal extends React.Component {
    ...
    componentWillMount() {
        ...
        axios.get(`${process.env.REACT_APP_MULE_HOST}api/sap/hcm/v2/ws/user/manager`, muleSoftConfig)
            .then(res => {
