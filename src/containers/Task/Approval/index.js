...
class ApprovalComponent extends React.Component {
  ...
  componentDidMount() {
    const params = `pageIndex=${Constants.TASK_PAGE_INDEX_DEFAULT}&pageSize=${Constants.TASK_PAGE_SIZE_DEFAULT}&status=${Constants.STATUS_WAITING}&fromDate=${moment().subtract(30, "days").format("YYYYMMDD")}&toDate=${moment().format("YYYYMMDD")}&`;
  ...
  render() {
    ...
    let statusFiler = [
      ...
      { value: Constants.STATUS_WORK_DAY_LOCKED_APPROVAL , label: t("PaidDayLocked") },
    ]
    return (
      <>
