...
const formatProcessTime = (time) => {
  ...
  return `${moment(time).format('DD/MM/YYYY')} | ${moment(time).format(
    'HH:mm:ss'
  )}`;
};
    ...
    <div className="row" style={{ rowGap: 18 }}>
      <div className="col-4">
        ...
        <input
          ...
          value={formatProcessTime(createdDate)}
          readOnly
        />
      </div>
      {formatProcessTime(coordinatorDate) && (
        <div className="col-4">
          <p className="title2">{t('SupportHRViewedSalaryDate')}</p>
          <input
            type="text"
            className="form-control"
            value={formatProcessTime(coordinatorDate)}
            readOnly
          />
        </div>
      )}
      {lastConsenter && formatProcessTime(lastConsenter.appraisalDate) && (
        <div className="col-4" key={lastConsenter.appraiserId}>
          <p className="title2">{`${t('ConsentDate')}`}</p>
          <input
            type="text"
            className="form-control"
            value={formatProcessTime(lastConsenter.appraisalDate)}
            readOnly
          />
        </div>
      )}
      {HRconsenter && formatProcessTime(HRconsenter.appraisalDate) && (
        <div className="col-4">
          <p className="title2">{`${t('HRAppraiseDate')}`}</p>
          <input
            type="text"
            className="form-control"
            value={formatProcessTime(HRconsenter.appraisalDate)}
            readOnly
          />
        </div>
      )}
      {formatProcessTime(approvedDate) && (
        <div className="col-4">
          <p className="title2">{t('ApprovalDate')}</p>
          <input
            type="text"
            className="form-control"
            value={formatProcessTime(approvedDate)}
            readOnly
          />
        </div>
      )}
