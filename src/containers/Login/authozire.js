    ...
    const hasPermissonShowPrepareTab = async (token, companyCode) => {
        try {
            ...
            if (response && response.data) {
                //return  response.data.data?.isSupporter == true || ( [Constants.pnlVCode.VinSchool, Constants.pnlVCode.VincomRetail, Constants.pnlVCode.VinHome, Constants.pnlVCode.Vin3S].includes(companyCode) && response.data.data?.hasSubordinate == true) ? true : false;
                return  response.data.data?.isSupporter == true || ( [Constants.pnlVCode.VinSchool, Constants.pnlVCode.VinHome, Constants.pnlVCode.Vin3S].includes(companyCode) && response.data.data?.hasSubordinate == true) ? true : false;
            }
            return false;
        } catch(e) {
            return false;
