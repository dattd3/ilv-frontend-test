      ...
      <div className="d-block clearfix">
        ...
        <div className="float-left text-wrap w-75">
          {props.isSearch === false && (
            <div className="title font-weight-bold text-dark">
              {t("AssesserRecently")}
            </div>
          )}
          ...
          <div className="comment">
            <i>
              ({props.data.account}) {props.data.current_position}
            </i>
          </div>
        </div>
      </div>
...
export default function SearchUserComponent({
  ...
  const handleInputChange = debounce((value) => {
    ...
    if (value !== "") {
      ...
      const payload = {
        account: value,
        employee_type: employeeSearchType,
      ...
      axios
        ...
        .then((res) => {
          if (res && res.data && res.data.data) {
            ...
            const lst =
              ignoreAccounts?.length > 0
                ? users.filter((user) => !ignoreAccounts.includes(user.account))
                : users;
            setUsers(lst);
          }
        })
  ...
  const filterOption = (option, inputValue) => {
    const options = (users || []).filter(
      (opt) =>
        ...
        opt.uid?.toLowerCase().includes(inputValue.toLowerCase())
    );
    return options;
  };
  ...
  return (
    <div className="appraiser">
      <div className="box shadow">
        <div className="row">
          <div className="col-12 col-xl-12">
            <div className="box-bottom">
              <b className="text-uppercase black-color">{title} </b>
              {/* <i className="text-danger">({t("AppraisalDuplicateApproval")})</i> */}
            </div>
          </div>
        </div>
        <div className="row">
          ...
          <div className="col-12 col-xl-4">
            ...
            <div>
              <input
                ...
                value={userSelected?.current_position || ""}
                readOnly
              />
            </div>
          </div>
        ...
        {
          !!approvalMatrixUrl && <div className="row business-type"><span className="col-12 text-info smaller font-14">* {t("NoteSelectApprover")} <b><a href={approvalMatrixUrl} target="_blank" >{t("ApprovalMatrix")}</a></b></span></div>
        }
      </div>
    </div>
  );
