...
class ApproverComponent extends React.Component {
  ...
  isApprover = (levelApproverFilter, orglv2Id, currentUserLevel, account) => {
    ...
    if (![Constants.pnlVCode.VinHome, Constants.pnlVCode.VincomRetail, Constants.pnlVCode.VinSchool, Constants.pnlVCode.VinMec, Constants.pnlVCode.VinFast, Constants.pnlVCode.VinFastTrading, Constants.pnlVCode.VinES].includes(currentUserPnLVCodeLogged)) {
      listLevelsApprover = Constants.APPROVER_LIST_LEVEL.filter((item, index) => index !== 0 && index !== 1)
    }
    ...
    if (
      indexApproverFilterLevel === -1 
      || indexCurrentUserLevel > indexApproverFilterLevel 
      || !listLevelsApprover.includes(levelApproverFilter) 
