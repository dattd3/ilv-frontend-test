...
const MyOption = props => {
    const { innerProps, innerRef, data, isShowRecentManagerLabel } = props;
    ...
    return (
        <div ref={innerRef} {...innerProps} className="approver" style={{ padding: '2px 0', fontSize: 13 }}>
            <div className="d-block clearfix">
                ...
                <div className="float-left text-wrap w-75">
                    {isShowRecentManagerLabel && <div className="title font-weight-bold text-dark" style={{ marginBottom: 4 }}>{t('AssesserRecently')}</div>}
                    <div className="title">{data?.fullName}</div>
                    <div className="comment" style={{ marginTop: 2 }}><i>({data?.account}) {data?.current_position}</i></div>
                </div>
            </div>
        </div>
    )
...
class AssesserComponent extends React.Component {
    constructor(props) {
        super();
        this.state = {
            appraiser: null,
        ...
        this.onInputChange = debounce(this.getAppraiser, 800);
    }
                ...
                <div className="row">
                    <div className="col-12 col-xl-12"> 
                        <div className="box-bottom"><b className="text-uppercase black-color">{t('CONSENTER')} </b>
                            {isShowDuplicateWarning && <i className="text-danger">({t('AppraisalDuplicateApproval')})</i>}
                        </div>
                    </div>
                </div>
                <div className="row">
                    <div className="col-12 col-xl-4">
                        ...
                        <div>
                            <Select
                                ...
                                isClearable={true}
                                styles={customStyles}
                                components={{ Option: e => MyOption({...e, isShowRecentManagerLabel: isShowRecentManagerLabel})}}
...
export default withTranslation()(AssesserComponent)
