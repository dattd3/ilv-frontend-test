...
export default class SearchMultiUsers extends Component {
    constructor(props) {
        ...
        this.onInputChange = debounce(this.searchUser, Constants.TIME_DEBOUNCE_FOR_SEARCH)
    }
    ...
    searchUser = (value) => {
        ...
        .replace(/\s/g, ",")
        .replace(/\,\,+/g, ",")
        const lisAccounts = account?.split(',')
        const maximumAccountToSearch = 200

        if ((lisAccounts || []).filter(item => item)?.length > maximumAccountToSearch) {
            return this.setState({
                    statusModal: {
                        isShow: true,
                        isSuccess: false,
                        content: `Chỉ cho phép nhập tối đa ${maximumAccountToSearch} mã NV hoặc AD`,
                    }
                })
        }
    ...
    customOption = (option) => {
        const employeeData = option.data
        return (
            <div
    ...
    handleSelectUsers = (users) => {
        if (users?.value === codeSelectAll) {
            this.props.handleSelectEmployee(this.state.listUsers)
        } else {
            this.props.handleSelectEmployee([users])
        }
    }
    ...
    render() {
        const { listUsers, isSearching, dropdownOpen, statusModal } = this.state
        const { handleDeleteEmployee, listEmployees, isDisabled } = this.props
        const hasEmployeeSelected = listEmployees?.length > 0
        const optionUsers = listUsers?.length > 0 ? [{ value: codeSelectAll, label: 'Chọn tất cả' }, ...listUsers] : []
        ...
        return (
            ...
            <StatusModal 
                ...
                textContent={statusModal?.content} 
                onHide={this.onHide} 
            />
            <div className="custom-dropdown" style={{background: `${isDisabled ? 'hsl(0,0%,95%)' : '#FFFFFF'}`}}>
                <div
                    className="d-flex align-items-center inner"
                ...
                >
                    ...
                    <div className="right-search d-flex">
                        ...
                        <Image
                            ...
                            style={{visibility: `${hasEmployeeSelected ? 'visible' : 'hidden'}`}}
                            src={IconCloseSmall}
                            alt="Delete"
                            onClick={() => handleDeleteEmployee()}
                        />
                        <div className="dropdown-icon"><Image src={IconDropDownSmall} alt="DropDown" /></div>
                    </div>
                </div>
                ...
                >
                    ...
                    <DropdownMenu>
                        ...
                        <div className="wrap-employee-table">
                            <table>
                                <thead>
                                    <tr>
